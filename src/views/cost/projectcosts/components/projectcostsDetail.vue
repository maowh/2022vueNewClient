<template>
  <div class="user-info-container">
    <div class="user-info-box" id="userInfoBox">
      <!-- 标题 -->
      <h2 class="title">{{ $t('msg.route.projectcostsInfo') }}</h2>

      <div class="header">
        <!-- 头部渲染表格 -->
        <el-descriptions :column="2" border>
          <el-descriptions-item :label="$t('msg.cost.projectName')">{{
            detailData.projectName
          }}</el-descriptions-item>
          <el-descriptions-item :label="$t('msg.cost.customerName')">{{
            detailData.customerName
          }}</el-descriptions-item>
          <el-descriptions-item :label="$t('msg.cost.businessLines')">{{
            detailData.businessLines
          }}</el-descriptions-item>
          <el-descriptions-item :label="$t('msg.cost.domain')">{{
            detailData.domain
          }}</el-descriptions-item>
          <el-descriptions-item :label="$t('msg.cost.year')">{{
            detailData.year
          }}</el-descriptions-item>
          <el-descriptions-item :label="$t('msg.cost.categoryName')">{{
            detailData.categoryName
          }}</el-descriptions-item>
          <el-descriptions-item :label="$t('msg.cost.incomeAmount')">{{
            detailData.incomeAmount
          }}</el-descriptions-item>
          <el-descriptions-item :label="$t('msg.cost.splitAmount')">{{
            detailData.splitAmount
          }}</el-descriptions-item>
          <el-descriptions-item :label="$t('msg.cost.RegularEmployees')">{{
            detailData.RegularEmployees
          }}</el-descriptions-item>
          <el-descriptions-item :label="$t('msg.cost.OutsourcedEmployees')">{{
            detailData.OutsourcedEmployees
          }}</el-descriptions-item>
          <el-descriptions-item :label="$t('msg.cost.PurchaseAmount')">{{
            detailData.PurchaseAmount
          }}</el-descriptions-item>
          <el-descriptions-item :label="$t('msg.cost.NonPurchaseAmount')">{{
            detailData.NonPurchaseAmount
          }}</el-descriptions-item>
          <el-descriptions-item :label="$t('msg.cost.TotalPurchaseAmount')">{{
            detailData.TotalPurchaseAmount
          }}</el-descriptions-item>
          <el-descriptions-item :label="$t('msg.cost.capitaIncome')">{{
            detailData.capitaIncome
          }}</el-descriptions-item>
        </el-descriptions>
      </div>

      <!-- 尾部签名 -->
      <div class="foot">
        <el-button type="text" class="edit" @click="onBackClick">{{
          $t('msg.article.back')
        }}</el-button>
        <el-button type="text" class="edit" @click="onEditClick">{{
          $t('msg.article.edit')
        }}</el-button>
      </div>
    </div>
    <!-- </el-card> -->
  </div>
</template>

<script setup>
import { ref } from 'vue'
import { costDisplay } from '@/api/cost'
import { useRoute, useRouter } from 'vue-router'

const route = useRoute()
const router = useRouter()
// 数据相关
const detailData = ref({})
const getCostDisplay = async () => {
  detailData.value = await costDisplay({
    table: 'projectcosts',
    id: route.params.id
  })
  detailData.value = detailData.value[0]
  console.log(detailData.value)
}
getCostDisplay()

const onBackClick = () => {
  router.push('/outsourcing/projectcosts')
}
const onEditClick = () => {
  router.push(`/outsourcing/projectcostsEdit/${route.params.id}`)
}
</script>

<style lang="scss" scoped>
.print-box {
  margin-bottom: 20px;
  text-align: right;
}
.user-info-box {
  width: 1024px;
  margin: 0 auto;
  .title {
    text-align: center;
    margin-bottom: 18px;
    margin-top: 30px;
  }
  .header {
    display: flex;
    ::v-deep .el-descriptions {
      flex-grow: 1;
    }

    .avatar {
      width: 187px;
      box-sizing: border-box;
      padding: 30px 20px;
      border: 1px solid #ebeef5;
      border-left: none;
    }
    .remark {
      margin-right: 12px;
    }
  }
  .body {
    ul {
      list-style: none;
      li {
        span {
          margin-right: 62px;
        }
      }
    }
  }
  .foot {
    margin-top: 20px;
    text-align: right;
    .edit {
      float: right;
      margin-right: 20px;
    }
  }
}
</style>

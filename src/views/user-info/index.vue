        ...
        <div class="header">
          ...
          <el-descriptions :column="2" border>
            ...
            <el-descriptions-item :label="$t('msg.userInfo.remark')" :span="2">
              <!-- <el-tag class="remark" size="small">{{
                detailData.title
              }}</el-tag> -->
              <el-tag
                class="remark"
                size="small"
                v-for="(item, index) in detailData.role"
                :key="index"
                >{{ item.title }}</el-tag
              >
            </el-descriptions-item>
...
import { userDetail } from '@/api/sys'
...
const getUserDetail = async () => {
  const detailDataTmp = await userDetail(props.id)
  detailData.value = detailDataTmp[0]
}
getUserDetail()
